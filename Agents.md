...
Detta dokument beskriver produktens mål, arkitektur, viktiga beslut och utvecklingsregler. AI-agenter ska läsa dokumentet och relevant befintlig kod innan ändringar föreslås eller genomförs.

> Förstå befintlig funktionalitet innan du ändrar den.

Min Gym App används i verklig träning. Stabilitet, korrekt data och enkelhet är viktigare än stora refaktoreringar eller teknisk elegans. Gör minsta rimliga ändring och bevara fungerande funktionalitet.

## 2. Produktöversikt

Min Gym App är en personlig styrketräningsapp som används som installerbar PWA, främst på iPhone.
...
* Supabase/PostgreSQL
* Supabase Authentication med Magic Link
* PostgreSQL RPC
...
Live Server kör lokal frontend mot samma Supabase-backend som produktionen.

> Lokal frontend, riktig backend.

Databasoperationer från Live Server kan påverka riktig data även om koden inte är committad eller pushad. Var försiktig med destruktiva tester.
...
`src/storage.js` är gränsen mellan frontendlogik och Supabase. Funktionsmoduler ska normalt använda `Storage` istället för att implementera egen databaslogik.
...
Aktiv Version 2-kod laddas huvudsakligen från `src/`. Äldre Version 1-filer kan finnas i projektroten med samma namn som filer under `src/`.

Betrakta dem som potentiellt legacy men radera aldrig filer enbart baserat på filnamn. Kontrollera först `index.html`, referenser från andra JavaScript-filer och vid behov Git-historik.
...
* initiering och navigation
* profil och appens rubrik
...
* autocomplete och stavningsnära förslag
* bevara `exercise_id`
...
* visa, skapa, redigera och ta bort
...
* lägga till extra övningar och set
* spara pass

**Kritisk regel:** Endast övningar markerade som genomförda ska sparas i historiken. En hoppad övning ska inte registreras som genomförd.

`Lägg till övning i dagens pass` är kritisk funktionalitet och ska fungera även om övningen inte finns i ursprungligt upplägg. Regressionstesta efter ändringar i sessionslogiken.

### `src/history.js`

Ansvarar för träningshistorik, tidigare pass och PB.
...
Gemensamma hjälpfunktioner, exempelvis lokala ID:n och HTML escaping. Lägg inte domänlogik här.
...
Ansvarar för Magic Link, login/logout, återställning av session och start av app efter autentisering.

### `workspace.js`

Ansvarar för aktivt/tillgängliga workspaces, workspace-byte, ihågkommet workspace, personligt workspace och `display_name`.
...
Workspace är den centrala modellen för ägarskap och delning av träningsdata. Varje vanlig användare har ett personligt workspace.
...
En användare kan tillhöra flera workspaces. Produktbegreppet är **medlem**, inte PT. En medlem kan exempelvis vara PT, träningskompis, partner eller annan person som hjälper till med träningen.

Alla medlemmar har för närvarande samma rättigheter. Inför inte avancerade roller utan konkret behov.
...
Nuvarande modell kräver att personen redan finns i Supabase Authentication. Användaradministration sker manuellt i Supabase Dashboard. Bygg inte eget administratörsgränssnitt för användarskapande utan nytt behov.

## 9. Workspace-UI och profil

Workspace-väljaren ska visa workspace-ägarens namn, inte tekniska ID:n. Om användaren bara har ett workspace behöver väljaren inte visas.

Valt workspace ska kunna kommas ihåg mellan omladdningar. Vid workspace-byte ska relevant data laddas från det nya workspacet.
...
Inte i `workspaces.name`. E-post används som fallback där det är lämpligt.

## 10. Träningsupplägg och pass

Upplägg är mallar för framtida träningspass och innehåller namn, övningar, set, målvikt och målrepetitioner.

Det faktiska träningsresultatet registreras i passet och sparas i historiken.

### Standardupplägg uppdateras inte automatiskt

Ett genomfört pass får inte automatiskt ändra standardupplägget. Resultatet sparas i historiken.

Vill användaren ändra standardvärden görs det manuellt via `Upplägg → Redigera`.
...
* vikt/reps ska motsvara genomfört pass
* historiska pass får inte förändras när upplägg senare redigeras
* snapshots används där historisk information måste bevaras

PB beräknas från historiken. Det finns ingen separat PB-tabell som källa till sanningen.
...
`exercise_library` används för konsekventa övningsidentiteter och datakvalitet. Fritext kan annars skapa flera namn för samma övning och felaktiga PB-resultat.

Implementerad modell:

* autocomplete
* befintliga övningar kan väljas
* stavningsnära namn föreslås men slås inte automatiskt ihop
...
Undvik ren namnmatchning som primär övningsidentitet. Konsekvent övningsdata är grund för framtida statistik.

## 13. Beslut som ska bevaras

### Offline

Offline-stöd är medvetet nedprioriterat eftersom WiFi/mobildata normalt finns, datamängderna är små och synk ökar komplexiteten. Bygg inte offline-synk och lova inte offline-stöd i UI utan nytt produktbeslut.

### Import

Excel/CSV-import ska inte byggas som appfunktion. Eventuell historisk import görs administrativt via Supabase/SQL. Visa inte framtida importfunktion i UI.

### Backup

Separat backupfunktion i UI är borttagen. Data lagras i Supabase. Återinför inte backupsektion utan konkret behov.

### Arkivering

Arkivering av gamla upplägg är en möjlig framtida funktion men inte prioriterad. Problemet är listans längd över tid, inte vem som skapade upplägget.
...
På iPhone finns ett känt UX-problem där Magic Link kan öppnas i Safari medan installerad PWA har ett annat sessionsläge.
...
Vid Auth-ändringar har tillfälliga fördröjningar observerats. Vid problem:

* kontrollera Console/loggar
* verifiera verkliga fel innan större ändringar
* undvik aggressiv felsökning om problemet rimligen kan vara tillfälligt

## 16. RLS och säkerhet

Row Level Security används. Säkerhetslogik får inte kringgås från frontend.
...
SQL-filer sparas i `database/` och fungerar som dokumentation och versionshistorik.

Den faktiska databasen ändras först när SQL körs i Supabase. Att endast skapa SQL-filen lokalt ändrar inte databasen.
...
GitHub används för versionshantering och GitHub Pages för produktion.
...
Git-commit påverkar endast kodhistoriken. Databasoperationer via appen skickas direkt till Supabase och är oberoende av Git.

## 19. Produktstatus och roadmap

Version 2.1 innehåller bland annat delade workspaces, medlemmar, medlemsinbjudan, borttagning av medlem, workspace-listning, workspace-väljare och `display_name`/e-post-fallback.
...
Övningsbank/autocomplete är viktigt för konsekventa övningsnamn, återanvändning av `exercise_library`, bättre PB-datakvalitet och framtida statistik.
...
`workspace` är ett tekniskt begrepp och bör inte dominera UI.

### Dialoger

Medlemsinbjudan använder egen HTML-dialog. Återgå inte till `window.prompt()` för medlemsinbjudan.
...
* starta pass
* ändra vikt/reps
* markera genomförd övning
* lämna övning omarkerad
* lägga till extra övning och set
* spara pass
...
* eget och delat workspace visas
* workspace-byte fungerar
* rätt upplägg, historik och medlemmar visas
...
* införa React/Vue/Svelte eller TypeScript
